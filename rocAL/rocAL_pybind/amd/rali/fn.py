...
def fish_eye_trail(*inputs, device=None):
    # pybind call arguments
    kwargs_pybind = {"input_image0": inputs[0],"is_output": False}
    fisheye_image = b.FishEye(Pipeline._current_pipeline._handle ,*(kwargs_pybind.values()))
    return (fisheye_image)

def brightness_trail(*inputs, brightness=1.0, bytes_per_sample_hint=0, image_type=0,
...
def resize_trail(*inputs, bytes_per_sample_hint=0, image_type=0, interp_type=1, mag_filter= 1, max_size = [0.0, 0.0], min_filter = 1,
            minibatch_size=32, preserve=False, resize_longer=0.0, resize_shorter= 0.0, resize_x = 0.0, resize_y = 0.0,
            save_attrs=False, seed=1, temp_buffer_hint=0, device = None):
...
def nop(*inputs, device=None, preserve = False):
    # pybind call arguments
    kwargs_pybind = {"input_image0": inputs[0],"is_output": False }
    nop_trandsformed_image = b.raliNop(Pipeline._current_pipeline._handle ,*(kwargs_pybind.values()))
    return (nop_trandsformed_image)
