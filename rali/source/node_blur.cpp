...
void BlurNode::create(std::shared_ptr<Graph> graph)
{
    ...
    _sdev.create_array(graph ,VX_TYPE_UINT32, _batch_size);
    _node = vxExtrppNode_BlurbatchPD(_graph->get(), _inputs[0]->handle(), _width_array,_height_array, _outputs[0]->handle(), _sdev.default_array(), _batch_size);
