...
// #include <stdio.h>
// #include <VX/vx.h>
// #include "OpenVX/OpenCV_Tunnel.h"
// #include "OpenVX/OpenCV_VX_Functions.h"
// #include "OpenVX/publishkernels.h"
// #include "OpenVX/vx_opencv.h"
// 
// // OpenVX Macros
// 
                                           ...
                                           const sensor_msgs::CameraInfoConstPtr& cam_info)
{
  ...
  //cv::imshow("what", visual_img);
  //cv::waitKey(2);
///////////////////////////////////////   Start OpenVX ORB Detection
//      vx_uint32 width = visual_img.cols;
//      vx_uint32 height = visual_img.rows;
//   
//      vx_status status = VX_SUCCESS;
//      vx_context context = vxCreateContext();
//  
//      if(context){
//          status = vxLoadKernels(context, "vx_opencv");
//          if(status == VX_SUCCESS){
//              vx_kernel kernel = vxGetKernelByName(context, "org.opencv.orb_detect");
//              if (kernel)
//              {
//                  vx_image input_rgb_image = vxCreateImage( context, width, height, VX_DF_IMAGE_RGB );
//                  vx_image output_image = vxCreateImage( context, width, height, VX_DF_IMAGE_U8 );
//  
//                  vx_array keypoints = vxCreateArray(context, VX_TYPE_KEYPOINT, 1000);
//                  ERROR_CHECK_OBJECT(keypoints);
//                  vx_delay keypointsDelay = vxCreateDelay( context, ( vx_reference )keypoints, 2 );
//  
...
//  
//                  // Trying to create the correct parameters for ORB
//                  vx_int32 features = 1000;
...
//                      // Creating an ORB node or medianBlur node
//                      vxExtCvNode_ORBdetect( graph, luma_image, luma_image, currentKeypoints, features, scaleFactor, nlevels, edgeThreshold, firstLevel, WTA_K, scoreType, patchSize)
//                 //     vxExtCvNode_medianBlur(graph, luma_image, output_image, 7)
//                  };
//                  
...
//                         cv_rgb_image_layout.stride_y   = 3840;
//                         vx_uint8 * cv_rgb_image_buffer = visual_img.data;
//  
...
//                          //  std::cout << kp_new_buf << std::endl;
//  
//                             for( vx_size i = 0; i < num_corners; i++ ){
//                               vx_keypoint_t * kp_new = (vx_keypoint_t *) ( kp_new_buf + i * kp_new_stride );
//                             //  float cordx = kp_new->x;
//                             //  float cordy = kp_new->y;
//                             //  float somes = 5;
//                           //    feature_locations_2d_.push_back((float)kp_new->x, (float)kp_new->y, (float)5);
//                               DrawPoint(visual_img, kp_new->x, kp_new->y);
//  
//                             }
//                            ERROR_CHECK_STATUS( vxUnmapArrayRange( currentKeypoints, kp_new_map ) );
//                         }
//                      //  ERROR_CHECK_STATUS( vxAgeDelay( keypointsDelay ) );
//                        cv::imshow("mydeadwindow", visual_img);
//                        cv::waitKey(2);
//                  // }
//              }
//  
