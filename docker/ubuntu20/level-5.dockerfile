...
RUN DEBIAN_FRONTEND=noninteractive apt-get -y install sqlite3 libsqlite3-dev libbz2-dev libssl-dev python-dev python3-dev autoconf automake libtool curl make g++ unzip && \
        ...
        wget https://github.com/ROCmSoftwarePlatform/MIOpen/archive/2.10.0.zip && unzip 2.10.0.zip && \
        cd MIOpen-2.10.0 && mkdir build && cd build && \
        #cd MIOpen-2.10.0 && sudo cmake -P install_deps.cmake --minimum && mkdir build && cd build && \ - deps install turned off
