...
def runTestCommand (platform, project) {
    ...
    if (platform.jenkinsLabel.contains('centos') || platform.jenkinsLabel.contains('ubuntu')) {
        ...
        conformaceHIP = 'AGO_DEFAULT_TARGET=GPU LD_LIBRARY_PATH=./lib ./bin/vx_test_conformance --filter=-*.VirtualArray:-HarrisCorners.*:-vxCanny.*:-*.ReplicateNode:-*.ImageContainmentRelationship:-*.MapRandomRemap:-*.OnRandomAndNatural:-GaussianPyramid.GraphProcessing:-HalfScaleGaussian.GraphProcessing:*.* | tee OpenVX-GPU-HIP-Conformance-log.md'
        moveFiles = 'mv *.md ../../'
    }
