...
void simple_net(engine::kind engine_kind, int times = 100) {
    ...
    net_args.push_back({{ZENDNN_ARG_SRC, conv1_dst_memory},
        {ZENDNN_ARG_DST, lrn1_dst_memory}});
    ...
    net_args.push_back({{ZENDNN_ARG_SRC, lrn1_dst_memory},
    ...
    // The conv algorithm used is direct. This is because the weights
    // are in the grouped format(goihw) and the gemm conv algorithm
    ...
    if (conv2_prim_desc.src_desc() != conv2_src_memory.get_desc()) {
        ...
        net_args.push_back({{ZENDNN_ARG_FROM, pool1_dst_memory},
            {ZENDNN_ARG_TO, conv2_src_memory}});
    }
    ...
    net_args.push_back({{ZENDNN_ARG_SRC, conv2_src_memory},
    ...
    net_args.push_back({{ZENDNN_ARG_SRC, conv2_dst_memory},
        {ZENDNN_ARG_DST, lrn2_dst_memory}});
    ...
    net_args.push_back({{ZENDNN_ARG_SRC, lrn2_dst_memory},
    ...
    write_to_zendnn_memory(conv3_weights.data(), conv3_user_weights_memory);
    auto conv3_user_bias_memory
    = memory({{conv3_bias_tz}, dt::f32, tag::x}, eng);
    write_to_zendnn_memory(conv3_bias.data(), conv3_user_bias_memory);
    ...
    auto conv3_desc = convolution_forward::desc(prop_kind::forward_inference,
                      algorithm::convolution_gemm, conv3_src_md, conv3_weights_md,
    ...
    if (conv3_prim_desc.src_desc() != conv3_src_memory.get_desc()) {
        ...
        net_args.push_back({{ZENDNN_ARG_FROM, pool2_dst_memory},
            {ZENDNN_ARG_TO, conv3_src_memory}});
    }
    ...
    net_args.push_back({{ZENDNN_ARG_SRC, conv3_src_memory},
        {ZENDNN_ARG_WEIGHTS, conv3_weights_memory},
        {ZENDNN_ARG_BIAS, conv3_user_bias_memory},
        {ZENDNN_ARG_DST, conv3_dst_memory}});
    ...
    net_args.push_back({{ZENDNN_ARG_SRC, conv3_dst_memory},
        {ZENDNN_ARG_DST, conv3_dst_memory}});
    ...
    write_to_zendnn_memory(conv4_weights.data(), conv4_user_weights_memory);
    auto conv4_user_bias_memory
    = memory({{conv4_bias_tz}, dt::f32, tag::x}, eng);
    write_to_zendnn_memory(conv4_bias.data(), conv4_user_bias_memory);
    ...
    if (conv4_prim_desc.src_desc() != conv4_src_memory.get_desc()) {
        ...
        net_args.push_back({{ZENDNN_ARG_FROM, conv3_dst_memory},
            {ZENDNN_ARG_TO, conv4_src_memory}});
    }
    ...
    net_args.push_back({{ZENDNN_ARG_SRC, conv4_src_memory},
        {ZENDNN_ARG_WEIGHTS, conv4_weights_memory},
        {ZENDNN_ARG_BIAS, conv4_user_bias_memory},
        {ZENDNN_ARG_DST, conv4_dst_memory}});
    ...
    net_args.push_back({{ZENDNN_ARG_SRC, conv4_dst_memory},
        {ZENDNN_ARG_DST, conv4_dst_memory}});
    ...
    write_to_zendnn_memory(conv5_weights.data(), conv5_user_weights_memory);
    auto conv5_user_bias_memory
    = memory({{conv5_bias_tz}, dt::f32, tag::x}, eng);
    write_to_zendnn_memory(conv5_bias.data(), conv5_user_bias_memory);
    ...
    if (conv5_prim_desc.src_desc() != conv5_src_memory.get_desc()) {
        ...
        net_args.push_back({{ZENDNN_ARG_FROM, conv4_dst_memory},
            {ZENDNN_ARG_TO, conv5_src_memory}});
    }
    ...
    net_args.push_back({{ZENDNN_ARG_SRC, conv5_src_memory},
        {ZENDNN_ARG_WEIGHTS, conv5_weights_memory},
        {ZENDNN_ARG_BIAS, conv5_user_bias_memory},
        {ZENDNN_ARG_DST, conv5_dst_memory}});
    ...
    net_args.push_back({{ZENDNN_ARG_SRC, conv5_dst_memory},
        {ZENDNN_ARG_DST, conv5_dst_memory}});
    ...
    net_args.push_back({{ZENDNN_ARG_SRC, conv5_dst_memory},
        {ZENDNN_ARG_DST, pool5_dst_memory}});
    ...
    write_to_zendnn_memory(fc6_weights.data(), fc6_user_weights_memory);
    auto fc6_user_bias_memory = memory({{fc6_bias_tz}, dt::f32, tag::x}, eng);
    write_to_zendnn_memory(fc6_bias.data(), fc6_user_bias_memory);
    ...
    if (fc6_prim_desc.src_desc() != fc6_src_memory.get_desc()) {
        ...
        net_args.push_back({{ZENDNN_ARG_FROM, pool5_dst_memory},
            {ZENDNN_ARG_TO, fc6_src_memory}});
    }
    ...
    net_args.push_back({{ZENDNN_ARG_SRC, fc6_src_memory},
        {ZENDNN_ARG_WEIGHTS, fc6_weights_memory},
        {ZENDNN_ARG_BIAS, fc6_user_bias_memory},
        {ZENDNN_ARG_DST, fc6_dst_memory}});
    ...
    write_to_zendnn_memory(fc7_weights.data(), fc7_user_weights_memory);

    auto fc7_user_bias_memory = memory({{fc7_bias_tz}, dt::f32, tag::x}, eng);
    write_to_zendnn_memory(fc7_bias.data(), fc7_user_bias_memory);

    // create memory descriptors for convolution data w/ no specified format
    auto fc7_bias_md = memory::desc({fc7_bias_tz}, dt::f32, tag::any);
    auto fc7_weights_md = memory::desc({fc7_weights_tz}, dt::f32, tag::nc);
    auto fc7_dst_md = memory::desc({fc7_dst_tz}, dt::f32, tag::nc);
    ...
    net_args.push_back({{ZENDNN_ARG_SRC, fc6_dst_memory},
        {ZENDNN_ARG_WEIGHTS, fc7_weights_memory},
        {ZENDNN_ARG_BIAS, fc7_user_bias_memory},
        {ZENDNN_ARG_DST, fc7_dst_memory}});
    ...
    write_to_zendnn_memory(fc8_weights.data(), fc8_user_weights_memory);
    auto fc8_user_bias_memory = memory({{fc8_bias_tz}, dt::f32, tag::x}, eng);
    write_to_zendnn_memory(fc8_bias.data(), fc8_user_bias_memory);
    auto user_dst_memory = memory({{fc8_dst_tz}, dt::f32, tag::nc}, eng);
    write_to_zendnn_memory(user_dst.data(), user_dst_memory);

    // create memory descriptors for convolution data w/ no specified format
    auto fc8_bias_md = memory::desc({fc8_bias_tz}, dt::f32, tag::any);
    auto fc8_weights_md = memory::desc({fc8_weights_tz}, dt::f32, tag::nc);
    auto fc8_dst_md = memory::desc({fc8_dst_tz}, dt::f32, tag::nc);
    ...
    net_args.push_back({{ZENDNN_ARG_SRC, fc7_dst_memory},
        {ZENDNN_ARG_WEIGHTS, fc8_weights_memory},
        {ZENDNN_ARG_BIAS, fc8_user_bias_memory},
        {ZENDNN_ARG_DST, fc8_dst_memory}});
    ...
    if (fc8_dst_memory != user_dst_memory) {
        ...
        net_args.push_back({{ZENDNN_ARG_FROM, fc8_dst_memory},
            {ZENDNN_ARG_TO, user_dst_memory}});
    }
...
int main(int argc, char **argv) {
    zendnnInfo(ZENDNN_TESTLOG, "zendnn_inference_f32 API test starts");
    try {
    ...
    zendnnInfo(ZENDNN_TESTLOG, "zendnn_inference_f32 API test ends\n");
    return 0;
}
