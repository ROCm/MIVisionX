...
Visualize::Visualize(Mat &image, vector<DetectedObject> &results) : mImage(image), mResults(results) {
...
void Visualize::show() {
	...
	for (int i = 0; i < detectedNum; i++) {
		...
		if (confidence > 30) {
			...
			rectangle(mImage, Point(left, top), Point(right, bottom), clr, 2);
			Size size = getTextSize(txt, FONT_HERSHEY_SIMPLEX, 0.6, 1, 0);
			int width = size.width;
			int height = size.height;
			rectangle(mImage, Point(left, (bottom - 5) - (height + 5)), Point((left + width), (bottom - 5)), clr, -1);
			putText(mImage, txt, Point((left + 5), (bottom - 10)), CV_FONT_HERSHEY_SIMPLEX, 0.6, Scalar(255, 255, 255), 1, 8);
		}
	}
...
void Visualize::LegendImage() {
	...
	for (int l = 0; l < 20; l++) {
		...
		putText(legend, className, Point(20, (l * 40) + 30), CV_FONT_HERSHEY_SIMPLEX, 0.6, Scalar(0, 0, 0), 1.5, 8);
