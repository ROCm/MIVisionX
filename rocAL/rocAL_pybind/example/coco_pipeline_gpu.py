...
# 1D labels & bboxes array
        if self.device == "cpu":
          ...
          torch_gpu_device = torch.device('cuda', self.device_id)
          boxes_array, labels_array = self.loader.raliGetEncodedBoxesAndLables(self.bs, self.num_anchors)
          self.encoded_bboxes = torch.as_tensor(boxes_array, device=torch_gpu_device)
          self.encoded_labels = torch.as_tensor(labels_array, device=torch_gpu_device)
...
def main():
    ...
    if(_rali_cpu):
        data_loader = RALICOCOIterator(
            pipe, multiplier=pipe._multiplier, offset=pipe._offset, display=display, device="cpu", num_anchors = len(dboxes)/4)
    else:
        data_loader = RALICOCOIterator(
            pipe, multiplier=pipe._multiplier, offset=pipe._offset, display=display, device="cuda", num_anchors = len(dboxes)/4)
    epochs = 2
