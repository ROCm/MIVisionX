...
RUN DEBIAN_FRONTEND=noninteractive apt-get -y install sqlite3 libsqlite3-dev libbz2-dev libssl-dev python-dev python3-dev python3-venv clang autoconf automake libtool curl make g++ unzip && \
        ...
        wget https://github.com/ROCmSoftwarePlatform/MIOpen/archive/2.12.0.zip && unzip 2.12.0.zip && \
        cd MIOpen-2.12.0 && sudo cmake -P install_deps.cmake --minimum && mkdir build && cd build && \
...
RUN DEBIAN_FRONTEND=noninteractive apt-get -y install libgflags-dev libgoogle-glog-dev liblmdb-dev nasm yasm libjsoncpp-dev clang && \
