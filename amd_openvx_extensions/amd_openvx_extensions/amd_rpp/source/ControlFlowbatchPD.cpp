...
struct ControlFlowbatchPDLocalData { 
...
static vx_status VX_CALLBACK refreshControlFlowbatchPD(vx_node node, const vx_reference *parameters, vx_uint32 num, ControlFlowbatchPDLocalData *data)
{
...
static vx_status VX_CALLBACK validateControlFlowbatchPD(vx_node node, const vx_reference parameters[], vx_uint32 num, vx_meta_format metas[])
{
	vx_status status = VX_SUCCESS;
	vx_enum scalar_type;
	...
	{
		return ERRMSG(VX_ERROR_INVALID_FORMAT, "validate: ControlFlowbatchPD: image: #0 format=%4.4s (must be RGB2 or U008)\n", (char *)&df_image);
	}
	...
	if(df_image != VX_DF_IMAGE_U8 && df_image != VX_DF_IMAGE_RGB) 
	{
		return ERRMSG(VX_ERROR_INVALID_FORMAT, "validate: ControlFlowbatchPD: image: #1 format=%4.4s (must be RGB2 or U008)\n", (char *)&df_image);
	}
...
static vx_status VX_CALLBACK processControlFlowbatchPD(vx_node node, const vx_reference * parameters, vx_uint32 num) 
{ 
	RppStatus status = RPP_SUCCESS;
	ControlFlowbatchPDLocalData * data = NULL;
	...
	if(data->device_type == AGO_TARGET_AFFINITY_GPU) {
		...
		if (df_image == VX_DF_IMAGE_U8 ){ 
 			// status = rppi_control_flow_u8_pln1_batchPD_gpu((void *)data->cl_pSrc1,(void *)data->cl_pSrc2,data->srcDimensions,data->maxSrcDimensions,(void *)data->cl_pDst,data->type,data->nbatchSize,data->rppHandle);
		}
		else if(df_image == VX_DF_IMAGE_RGB) {
			// status = rppi_control_flow_u8_pkd3_batchPD_gpu((void *)data->cl_pSrc1,(void *)data->cl_pSrc2,data->srcDimensions,data->maxSrcDimensions,(void *)data->cl_pDst,data->type,data->nbatchSize,data->rppHandle);
		}
		return status;
#endif
	}
	if(data->device_type == AGO_TARGET_AFFINITY_CPU) {
		refreshControlFlowbatchPD(node, parameters, num, data);
		if (df_image == VX_DF_IMAGE_U8 ){
			// status = rppi_control_flow_u8_pln1_batchPD_host(data->pSrc1,data->pSrc2,data->srcDimensions,data->maxSrcDimensions,data->pDst,data->type,data->nbatchSize,data->rppHandle);
		}
		else if(df_image == VX_DF_IMAGE_RGB) {
			// status = rppi_control_flow_u8_pkd3_batchPD_host(data->pSrc1,data->pSrc2,data->srcDimensions,data->maxSrcDimensions,data->pDst,data->type,data->nbatchSize,data->rppHandle);
		}
		return status;
	}
}

static vx_status VX_CALLBACK initializeControlFlowbatchPD(vx_node node, const vx_reference *parameters, vx_uint32 num) 
{
	ControlFlowbatchPDLocalData * data = new ControlFlowbatchPDLocalData;
...
static vx_status VX_CALLBACK uninitializeControlFlowbatchPD(vx_node node, const vx_reference *parameters, vx_uint32 num)
{
...
vx_status ControlFlowbatchPD_Register(vx_context context)
{
	vx_status status = VX_SUCCESS;
	// Add kernel to the context with callbacks
	vx_kernel kernel = vxAddUserKernel(context, "org.rpp.ControlFlowbatchPD",
		VX_KERNEL_RPP_CONTROLFLOWBATCHPD,
		processControlFlowbatchPD,
		8,
		validateControlFlowbatchPD,
		initializeControlFlowbatchPD,
		uninitializeControlFlowbatchPD);
