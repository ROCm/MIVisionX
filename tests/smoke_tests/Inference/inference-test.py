...
if os.path.exists(testSummaryDir):
    ...
    if os.system(cmd) != 0:
        print 'ERROR: Unable to delete directory ' + testSummaryDir    
cmd = 'mkdir ' + testSummaryDir
if os.system(cmd) != 0:
    print 'ERROR: Unable to create directory ' + testSummaryDir
...
for batchsize in batchsizes:
    cmd = 'python inference-performance-accuracy.py -n ' + network + ' -b ' + str(batchsize) + ' -f ' + str(FP16) + ' -c ' + str(runMode) + ' -a ' + str(argmaxSwitch) + ' -p ' + str(pythonMode) + ' >> ' + dumpFile
    if os.system(cmd) != 0:
...
cmd = 'python inference-parse-perf-dump.py ' + dumpFile + ' > ' + summaryFile
if os.system(cmd) != 0:
...
#Run accuracy script
if pythonMode == 1:
    cmd = 'python inference-measure-accuracy.py -d ' + testSummaryDir + ' -p 1' 
    if os.system(cmd) != 0:
        print 'ERROR: Could not run inference-measure-accuracy.py on ' + testSummaryDir + ' folder.\n'    
else:
    cmd = 'python inference-measure-accuracy.py -d ' + testSummaryDir
    if os.system(cmd) != 0:
        print 'ERROR: Could not run inference-measure-accuracy.py on ' + testSummaryDir + ' folder.\n'
