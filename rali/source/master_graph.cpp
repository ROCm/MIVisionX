            ...
            if(format == RaliTensorFormat::NHWC)
            {
                ...
                else if(output_data_type == RaliTensorDataType::FP16)
                {
                    ...
                    for(unsigned channel_idx = 0; channel_idx < c; channel_idx++)
                        for(unsigned i = 0; i < channel_size; i++)
                            output_tensor_16[dest_buf_offset+channel_idx+ i*c] =
                                    offset[channel_idx] + multiplier[channel_idx]*(reverse_channels ? (half)(in_buffer[(c-channel_idx-1)*channel_size+i]) : (half)(in_buffer[channel_idx*channel_size + i]));
                }
            }
            if(format == RaliTensorFormat::NCHW)
            {
                if(output_data_type == RaliTensorDataType::FP32)
                {
                    ...
                    if(c != 3)
                    {
                        for(unsigned channel_idx = 0; channel_idx < c; channel_idx++)
                            for(unsigned i = 0; i < channel_size; i++)
                                output_tensor_32[dest_buf_offset+channel_idx*channel_size + i] =
                                        offset[channel_idx] + multiplier[channel_idx]*(reverse_channels ? (float)(in_buffer[(c-channel_idx-1)*channel_size+i]) : (float)(in_buffer[channel_idx*channel_size + i]));
                    }
                    else {
#if (ENABLE_SIMD && __AVX2__)
                        ...
                        for (; i < alignedLength; i += 8) {
                            ...
                            if (reverse_channels) {
                                pixG = _mm_loadl_epi64((const __m128i *) in_buffer_R);
                ...
                else if(output_data_type == RaliTensorDataType::FP16)
                {
                    ...
                    for(unsigned channel_idx = 0; channel_idx < c; channel_idx++)
                        for(unsigned i = 0; i < channel_size; i++)
                            output_tensor_16[dest_buf_offset+channel_idx*channel_size + i] =
                                    offset[channel_idx] + multiplier[channel_idx]*(reverse_channels ? (half)(in_buffer[i+(c-channel_idx-1)*channel_size]) : (half)(in_buffer[i+channel_idx*channel_size]));
                }
