  ...
  cam_info_(*cam_info)
{
 ...
 // cv::imshow("mywindow", gray_img);
 // cv::waitKey(2);

 // cv::Mat myImage = visual.clone();
  ...
  cv::Mat points_img = visual.clone();

///////////////////////////////////////   Start OpenVX ORB Detection

  // Determine what part of the image to be passed into OpenVX graph
  vx_rectangle_t cv_rgb_image_region;
  cv_rgb_image_region.start_x    = 0;
  cv_rgb_image_region.start_y    = 0;
  cv_rgb_image_region.end_x      = image_width;
  cv_rgb_image_region.end_y      = image_height;
  vx_imagepatch_addressing_t cv_rgb_image_layout;
  cv_rgb_image_layout.stride_x   = 3;
  cv_rgb_image_layout.stride_y   = visual.step;
  vx_uint8 * cv_rgb_image_buffer = visual.data;

  // Copy that part of the image into the OpenVX Kernel for processing
  ERROR_CHECK_STATUS( vxCopyImagePatch( input_rgb_image, &cv_rgb_image_region, 0,
                                         &cv_rgb_image_layout, cv_rgb_image_buffer,
                                         VX_WRITE_ONLY, VX_MEMORY_TYPE_HOST ) );

  // Process graph
  ERROR_CHECK_STATUS( vxProcessGraph( graph_vx ) );

  vx_size num_corners = 0, num_tracking = 0;
  // Get delay for keypoints
  currentKeypoints  = ( vx_array )vxGetReferenceFromDelay( keypointsDelay, 0 );

  ERROR_CHECK_OBJECT( currentKeypoints );
  ERROR_CHECK_STATUS( vxQueryArray( currentKeypoints, VX_ARRAY_NUMITEMS, &num_corners, sizeof( num_corners ) ) );
  ...
  if( num_corners > 0 ){
    ...
    for( vx_size i = 0; i < num_corners; i++ ){
      ...
      // Store keypoints into RGBDSLAM
      feature_locations_2d_.push_back(cv::KeyPoint((float)kp_new->x, (float)kp_new->y, 30));
    }
    ERROR_CHECK_STATUS( vxUnmapArrayRange( currentKeypoints, kp_new_map ) );
    //std::cout<<"Number of ORBs : ---------------------------------------->"<<feature_locations_2d_.size()<<std::endl;
  }
  
//////////////////////////////////////    End OpenVX ORB Detection
...
unsigned int Node::featureMatching(const Node* other, std::vector<cv::DMatch>* matches) const 
{
  ...
  if (ps->get<std::string> ("matcher_type") != "BRUTEFORCE_OPENCV" && ps->get<std::string> ("feature_extractor_type") == "ORB")
  {
    ...
    //----------------------------------------------OpenCL Implementation of BruteForce------------------------------------------------------

    if(ps->get<std::string> ("feature_extractor_type") == "ORB")
    {
        ScopedTimer s("My bruteforce Search", false, true);

        // Query value is the array containing feature descriptors of the current node
        // Search array is the array containing feature descriptors of one of the neighbor nodes
        ...
        int query_size = this->feature_locations_2d_.size();
        int search_size = other->feature_locations_2d_.size()-1;

        // Match array will store the output of the BruteForce OpenCL kernel and will be stored into
        // a vector of cv::DMatch
        int *match_array = (int *)malloc(query_size * 3 * sizeof(int));

        // Declare global and local work sizes for OpenCL
        size_t global_work_size, local_work_size;

        // Buffers required to pass data into OpenCL kernel
        cl_mem query_buff, search_buff, min_buff, result_buff, match_buff, return_buff;
        query_buff = search_buff = min_buff = result_buff = match_buff = return_buff = NULL;

        // Three consecutive kernels are used
		cl_kernel kernel_1 = NULL;
	  	cl_kernel kernel_2 = NULL;
	  	cl_kernel kernel_3 = NULL;
		...
		// Create the required memory buffers
		query_buff   = clCreateBuffer(context, CL_MEM_READ_ONLY,  (4 * query_size * sizeof(uint64_t)), NULL, &ret);
		search_buff  = clCreateBuffer(context, CL_MEM_READ_ONLY,  (4 * search_size * sizeof(uint64_t)), NULL, &ret);
		min_buff     = clCreateBuffer(context, CL_MEM_READ_WRITE, (query_size * sizeof(int)), NULL, &ret);
		result_buff  = clCreateBuffer(context, CL_MEM_READ_WRITE, (query_size * sizeof(int)), NULL, &ret);
		match_buff   = clCreateBuffer(context, CL_MEM_WRITE_ONLY, (query_size * 3 * sizeof(int)), NULL, &ret);
		return_buff  = clCreateBuffer(context, CL_MEM_WRITE_ONLY, (query_size *search_size * sizeof(int)), NULL, &ret);

		// Write input memory buffers - query value and search array to device memory
		ret  = clEnqueueWriteBuffer (command_queue, query_buff,  CL_TRUE, 0, (4 * query_size * sizeof(uint64_t)),  (void *)query_value,  0, NULL, NULL);
		ret |= clEnqueueWriteBuffer (command_queue, search_buff, CL_TRUE, 0, (4 * search_size * sizeof(uint64_t)), (void *)search_array, 0, NULL, NULL);
		if (ret != CL_SUCCESS)
			printf("Failed to copy date from host to device: %d\n", (int) ret);

		// Create OpenCL Kernel 1 for BruteForce and finding the hamming distances between each
		// feature descriptor in query value with each one in search array
		kernel_1 = clCreateKernel(program, "bruteforce_hamming", &ret);
		if (ret != CL_SUCCESS)
			printf("Failed to create kernel %d\n", (int) ret);
	
		// Create OpenCL Kernel 2 to find the minimum hamming distance for each value in query value
		kernel_2 = clCreateKernel(program, "bruteforce_min", &ret);
		if (ret != CL_SUCCESS) 
			printf("Failed to create kernel %d\n", (int) ret);

		// Create OpenCL Kernel 3 to write these minimum distances and their indices to the match
		// array buffer which will be passed back in to the host
		kernel_3 = clCreateKernel(program, "bruteforce_match", &ret);
		if (ret != CL_SUCCESS)
			printf("Failed to create kernel %d\n", (int) ret);

		// Set Kernel Arguments for all 3 kernels
		ret  = clSetKernelArg(kernel_1, 0, sizeof(cl_mem), (void *)&query_buff);
		ret |= clSetKernelArg(kernel_1, 1, sizeof(cl_mem), (void *)&search_buff);
		ret |= clSetKernelArg(kernel_1, 2, sizeof(cl_int), (void *)&query_size);
		ret |= clSetKernelArg(kernel_1, 3, sizeof(cl_int), (void *)&search_size);
		ret |= clSetKernelArg(kernel_1, 4, sizeof(cl_mem), (void *)&min_buff);
		ret |= clSetKernelArg(kernel_1, 5, sizeof(cl_mem), (void *)&result_buff);
		ret |= clSetKernelArg(kernel_1, 6, sizeof(cl_mem), (void *)&return_buff);
		ret |= clSetKernelArg(kernel_2, 0, sizeof(cl_int), (void *)&query_size);
		ret |= clSetKernelArg(kernel_2, 1, sizeof(cl_int), (void *)&search_size);
		ret |= clSetKernelArg(kernel_2, 2, sizeof(cl_mem), (void *)&min_buff);
		ret |= clSetKernelArg(kernel_2, 3, sizeof(cl_mem), (void *)&result_buff);
		ret |= clSetKernelArg(kernel_2, 4, sizeof(cl_mem), (void *)&return_buff);
		ret |= clSetKernelArg(kernel_3, 0, sizeof(cl_int), (void *)&query_size);
		ret |= clSetKernelArg(kernel_3, 1, sizeof(cl_int), (void *)&search_size);
		ret |= clSetKernelArg(kernel_3, 2, sizeof(cl_mem), (void *)&min_buff);
		ret |= clSetKernelArg(kernel_3, 3, sizeof(cl_mem), (void *)&result_buff);
		ret |= clSetKernelArg(kernel_3, 4, sizeof(cl_mem), (void *)&match_buff);
		if (ret != CL_SUCCESS)
			printf("Failed to set kernel arguments %d\n", (int) ret);

		// Define a 1-dimensional local work size as the product of query size or search size or 256,
		// whichever is lesser, as the local work size cannot be greater than 256 on this particular
		// device		
		local_work_size  = (size_t)((query_size * search_size) < 256 ? (query_size * search_size) : 256);
		global_work_size = (size_t)(ceil((query_size * search_size) / (float) local_work_size) * local_work_size);

		// Execute kernel 1
		ret = clEnqueueNDRangeKernel(command_queue, kernel_1, 1, NULL, &global_work_size, &local_work_size, 0, NULL, NULL);
		if (ret != CL_SUCCESS) 
			printf("Failed to execute kernel 1 for execution %d\n", (int) ret);

		// The next two kernels only need query size as the local work size
		local_work_size  = (size_t)((query_size) < 256 ? (query_size) : 256);
		global_work_size = (size_t)(ceil((query_size) / (float) local_work_size) * local_work_size);

		// Execute kernel 2
		ret = clEnqueueNDRangeKernel(command_queue, kernel_2, 1, NULL, &global_work_size, &local_work_size, 0, NULL, NULL);
		if (ret != CL_SUCCESS)
			printf("Failed to execute kernel 2 for execution %d\n", (int) ret);

		local_work_size  = (size_t)((query_size) < 256 ? (query_size) : 256);
		global_work_size = (size_t)(ceil((query_size) / (float) local_work_size) * local_work_size);

		// Execute kernel 3
		ret = clEnqueueNDRangeKernel(command_queue, kernel_3, 1, NULL, &global_work_size, &local_work_size, 0, NULL, NULL);
		if (ret != CL_SUCCESS)
			printf("Failed to execute kernel 3 for execution %d\n", (int) ret);

		// Read the result - match_array back from the device to the host
		ret  = clEnqueueReadBuffer(command_queue, match_buff, CL_TRUE, 0, (3 * query_size * sizeof(int)), (void *)match_array, 0, NULL, NULL);
		if (ret != CL_SUCCESS) 
			printf("Failed to copy data from device to host %d\n", (int) ret);

		// Write the contents of match_array to a vector of cv::DMatch if the result index is != -1
        for(int i = 0; i < query_size; i++)
        {
        	if(match_array[i * 3 + 1] != -1)
        	{
        		cv::DMatch match(match_array[i * 3 + 0], match_array[i * 3 + 1], match_array[i * 3 + 2] /256.0 + (float)rand()/(1000.0*RAND_MAX));
	    ...
	    clReleaseKernel(kernel_3);
	  	...
	  	clReleaseMemObject(query_buff);
	  	clReleaseMemObject(search_buff);
	  	clReleaseMemObject(min_buff);
	  	clReleaseMemObject(result_buff);
	  	clReleaseMemObject(match_buff);
	  	clReleaseMemObject(return_buff);

	  	free(match_array);
    }

    //------------------------------------------------End OpenCL BruteForce-------------------------------------------------------------------------------
...
{ 
  ...
  //std::cout << "global a is: " << get_a() << std::endl;
  inliers.clear();
