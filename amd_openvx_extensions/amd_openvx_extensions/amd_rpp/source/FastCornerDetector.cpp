...
static vx_status VX_CALLBACK refreshFastCornerDetector(vx_node node, const vx_reference *parameters, vx_uint32 num, FastCornerDetectorLocalData *data)
{
	...
	STATUS_ERROR_CHECK(vxReadScalarValue((vx_scalar)parameters[2], &data->noOfPixels));
	STATUS_ERROR_CHECK(vxReadScalarValue((vx_scalar)parameters[3], &data->threshold));
	STATUS_ERROR_CHECK(vxReadScalarValue((vx_scalar)parameters[4], &data->nonMaxKernelSize));
...
{ 
	...
	if(data->device_type == AGO_TARGET_AFFINITY_GPU) {
		...
		if (df_image == VX_DF_IMAGE_U8 ){ 
 			status = rppi_fast_corner_detector_u8_pln1_gpu((void *)data->cl_pSrc,data->srcDimensions,(void *)data->cl_pDst,data->noOfPixels,data->threshold,data->nonMaxKernelSize,data->rppHandle);
		}
	...
	if(data->device_type == AGO_TARGET_AFFINITY_CPU) {
		refreshFastCornerDetector(node, parameters, num, data);
		if (df_image == VX_DF_IMAGE_U8 ){
			status = rppi_fast_corner_detector_u8_pln1_host(data->pSrc,data->srcDimensions,data->pDst,data->noOfPixels,data->threshold,data->nonMaxKernelSize,data->rppHandle);
		}
		else if(df_image == VX_DF_IMAGE_RGB) {
