# OpenVX Vision Performance Tests

Benchmarks 116 OpenVX vision functions using `RunVX` across the `CPU`, `HIP`, and `OCL` backends. Each test measures per-frame execution time over a configurable number of frames.
...
Arguments:
  ...
  --hardware_mode    Execution target            (default: CPU  | options: CPU/GPU)
  --list_tests       Print the test list         (default: no   | options: no/yes)
  --test_filter      Run a single test by ID     (default: 0 = all | range: 1-N)
  --num_frames       Frames per test             (default: 1000 | range: 1-N)
  --backend_type     OpenVX backend              (default: CPU  | options: CPU/HIP/OCL)
  --profiling        Enable rocprof GPU profiling (default: no  | options: yes/no)
```

Example — run all tests on the HIP backend with rocprof profiling enabled:

```shell
python runVisionTests.py --runvx_directory /opt/rocm/bin --backend_type HIP --profiling yes
```
