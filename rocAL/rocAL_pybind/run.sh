...
if [ -n "$CONDA_DEFAULT_ENV" ]  || [ -n "$VIRTUAL_ENV" ] || [[ "$DEFAULT_PYTHON" == *"$CONDA"* ]]; then ## Checks if it is in any env then removes packages accordingly
    ...
    # INSTALL_FILE_PATH_ROCAL="/site-packages/amd_rocal-1.1.0-py$PYTHON_VERSION-linux-x86_64.egg"
...
if [[ $# -eq 1 ]]; then
  ...
  if [[ "$1" == "--backend_ocl" ]] || [[ "$1" == "--backend_hip" ]]; then
    ...
    pip$PYTHON_VERSION install ./dist/amd_rocal-1.1.0-cp39-cp39-linux_x86_64.whl
  else
    echo
